...
PACKET_HANDLER(
    ePacketType::LIGHTNING_EVENT, Packets::World::LightningEvent* pLightningEvent, CNetworkPlayer* pNetworkPlayer)
{
    if (!pLightningEvent || !pNetworkPlayer)
    {
        return;
    }

    GetPacketFactory().SendToAll(*pLightningEvent, pNetworkPlayer);
}
