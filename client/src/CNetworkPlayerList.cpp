...
{
	CRect rect = CRect(CUtil::SCREEN_STRETCH_X(fX), CUtil::SCREEN_STRETCH_Y(fY), CUtil::SCREEN_STRETCH_X(fX + BOX_WIDTH), CUtil::SCREEN_STRETCH_Y(fY + BOX_HEIGHT));
	...
	CFont::PrintString(
		rect.left + CUtil::SCREEN_STRETCH_X(8.0f), fmin(rect.bottom, rect.top) - CUtil::SCREEN_STRETCH_Y(10.0f), "Players");
}
...
// 	CRect rect = CRect(
// 		CUtil::SCREEN_STRETCH_X(fX + PING_OFFSET_X),
// 		CUtil::SCREEN_STRETCH_Y(fY + PING_OFFSET_Y),
...
// 	CFont::PrintString(CUtil::SCREEN_STRETCH_X(fX + PING_OFFSET_X + PING_COUNT_OFFSET_X), rect.top + CUtil::SCREEN_STRETCH_Y(PING_COUNT_OFFSET_Y), std::to_string(nPing).c_str());
// }

void CNetworkPlayerList::DrawName(CNetworkPlayer* pNetworkPlayer, float fX, float fY)
{
	char szPlayerName[24 + 1];
	...
	float fNormalizedValue = static_cast<float>(strlen(szPlayerName) - 1) / static_cast<float>(sizeof(szPlayerName) / sizeof(char) - 1 - 1);
	...
	CFont::PrintString(CUtil::SCREEN_STRETCH_X(fX + NAME_OFFSET_X), CUtil::SCREEN_STRETCH_Y(fY + NAME_OFFSET_Y) + CUtil::SCREEN_STRETCH_Y(4.0f + MAX_NAME_OFFSET_Y * fNormalizedValue), szPlayerName);
}
...
void CNetworkPlayerList::DrawBars(CPlayerPed* pPlayerPed, float fX, float fY)
{
	...
	if (pPlayerPed->m_fArmour != 0.0f)
	{
		CSprite2d::DrawBarChart(fBarOffsetX + barWidth, fBarOffsetY - BAR_OFFSET_Y - BAR_ARMOUR_OFFSET_Y, barWidth,
			barHeight, pPlayerPed->m_fArmour, 0, 0, 1, HudColour.GetRGBA(HUD_COLOUR_WHITE),
			CRGBA(0, 0, 0, 0));
	}

	CSprite2d::DrawBarChart(fBarOffsetX + barWidth, fBarOffsetY, barWidth,
		barHeight, pPlayerPed->m_fHealth, 0, 0, 1, HudColour.GetRGBA(HUD_COLOUR_RED),
		CRGBA(0, 0, 0, 0));
}
...
void CNetworkPlayerList::DrawWeaponIcon(CPlayerPed* pPlayerPed, float fX, float fY)
{
	...
	int nModelId = CUtil::GetWeaponModelById(pPlayerPed->m_aWeapons[pPlayerPed->m_nActiveWeaponSlot].m_eWeaponType);
	...
	RwTexture* texture = RwTexDictionaryFindHashNamedTexture(txd->m_pRwDictionary, CKeyGen::AppendStringToKey(mi->m_nKey, "ICON"));
	if (texture == nullptr)
	{
		return;
	}
	...
	CSprite::RenderOneXLUSprite(
		fOffsetX + fHalfWidth, fOffsetY + fHalfHeight, 1.0f,
...
void CNetworkPlayerList::DrawSeparator(float fCenterBoxX, float fCenterBoxY, float fColumnY)
{
	...
	CSprite2d::DrawRect(CRect(CUtil::SCREEN_STRETCH_X(fSeparatorX), CUtil::SCREEN_STRETCH_Y(fSeparatorY), CUtil::SCREEN_STRETCH_X(fSeparatorX + BOX_WIDTH + SEPARATOR_WIDTH), CUtil::SCREEN_STRETCH_Y(fSeparatorY + SEPARATOR_HEIGHT)), CRGBA(169, 169, 169, 130));
}
