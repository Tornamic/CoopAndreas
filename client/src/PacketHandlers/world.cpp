...
{
    // unused for now
    // CExplosion::AddExplosion(nullptr, nullptr, pAddExplosion->type, pAddExplosion->pos, pAddExplosion->time,
    // pAddExplosion->usesSound, pAddExplosion->cameraShake, pAddExplosion->isVisible);
}
...
PACKET_HANDLER(ePacketType::UPDATE_ALL_TAGS, Packets::World::UpdateAllTags* pUpdateAllTags)
{
    ...
    for (auto& tag : pUpdateAllTags->tags)
    {
        for (auto& tagDesc : CTagManager::ms_tagDesc)
        {
            ...
            if (static_cast<int16_t>(floor(pos.x)) == tag.pos_x && static_cast<int16_t>(floor(pos.y)) == tag.pos_y &&
...
PACKET_HANDLER(ePacketType::WANTED_SYNC, Packets::World::GameWantedSync* pWantedSync)
{ CWantedSync::HandlePacket(pWantedSync); }

PACKET_HANDLER(ePacketType::MONEY_SYNC, Packets::World::GameMoneySync* pMoneySync)
{ CMoneySync::HandlePacket(pMoneySync); }

PACKET_HANDLER(ePacketType::CHEAT_SYNC, Packets::World::GameCheatSync* pCheatSync)
{ CCheatSync::HandlePacket(pCheatSync); }